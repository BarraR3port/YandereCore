...
public class ChatClear implements ILyCommand {
    ...
    public CommandResponse command(CommandContext context){
        ...
        if (context.getArgs().length == 0){
            ...
            Utils.sendMessage(p, " ");
            Utils.sendMessage(p, Utils.formatTC("  &8&l▸ "), Utils.hoverOverMessageSuggestCommand("&c/chatclear &e<jugador>", Arrays.asList("&7Con este comando le limpias", "&7el chat a un jugador específico."), "/chatclear "));
            Utils.sendMessage(p, Utils.formatTC("  &8&l▸ "), Utils.hoverOverMessageSuggestCommand("&c/chatclear own", Arrays.asList("&7Con este comando limpias", "&7tu propio chat."), "/chatclear own"));
            Utils.sendMessage(p, Utils.formatTC("  &8&l▸ "), Utils.hoverOverMessageSuggestCommand("&c/chatclear all", Arrays.asList("&7Con este comando le limpias", "&7teleport an specific location."), "/chatclear all"));
            Utils.sendMessage(p, Utils.formatTC("  &8&l▸ "), Utils.hoverOverMessageSuggestCommand("&c/chatclear all -s", Arrays.asList("&7Con este comando le limpias", "&7teleport an specific location."), "/chatclear all -s"));
            return new CommandResponse();
